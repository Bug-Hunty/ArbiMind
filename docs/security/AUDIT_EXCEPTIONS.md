...
| [GHSA-r292-9mhp-454m](https://github.com/advisories/GHSA-r292-9mhp-454m) | high | tar | `packages/backend` and `packages/bot` > `@tensorflow/tfjs-node@4.22.0` > `tar@7.5.11` | Current parent is still `tfjs-node@4.22.0`; no supported parent release fixes the path. Patched `tar` is `>=7.5.21`, but this repository prohibits forcing a transitive tar override because it triggers full lockfile re-resolution. The path is install/native-addon tooling, not an application request path. Review by **2026-10-10**. |
